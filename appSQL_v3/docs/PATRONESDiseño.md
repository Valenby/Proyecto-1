...
adaptaciones para manejar algo de forma estadar de todos, haciendo que las funcionaldades queden igual. 
lo puedes usar como quieras, la unic aocndicion es que el objeto debe de ser privado. 

adapta las propiedades del objeto dependiendo al requerimiento, siempre y cuando sus valores sean privados.
```javascript 
class User {
    constructor(username: 'vale', passw: '123'){
        this._username = username;
        this.passw = encrypt(passw);
    }
    setPasswowrd(newPass) {
        this._passw = encrypt(passw);
    }
...
middlewares, patron de diseño uwu.
pdemos usar varios funcionamientos.
se puede reutilizar:
